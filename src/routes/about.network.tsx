    ...
    <div ref={ref} className="space-y-6 text-left">
      {bars.map((b) => (
        <div key={b.label}>
          ...
          <div className="h-3 w-full rounded-full bg-haze overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
...
function BusinessNetworkPage() {
  ...
  return (
    <>
      {/* Hero Section */}
      <section className="relative isolate overflow-hidden bg-navy pt-32 pb-20 md:py-36 min-h-[55vh] flex items-center">
        <div className="mx-auto max-w-7xl px-6 w-full">
          ...
          <p className="mt-4 max-w-[580px] text-lg text-white/80 leading-relaxed">
      ...
      <section className="bg-white py-12 border-b border-haze">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {stats.map((s) => (
              <div key={s.label}>
                <div className="font-display text-3xl sm:text-4xl font-bold text-gold">{s.num}</div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-16 lg:grid-cols-2 items-center">
            ...
            <Reveal delay={0.2} className="text-left">
              ...
              <p className="mt-4 text-[16px] leading-[1.85] text-mist">
              ...
              <div className="mt-8 space-y-6">
                {flowSteps.map((step) => (
                  <div key={step.num} className="flex items-start gap-4">
                    <span className="font-display text-2xl font-bold text-gold shrink-0">
                      {step.num}
                    </span>
                    <div>
                      <h4 className="font-display text-lg font-bold text-ink">{step.title}</h4>
                      <p className="mt-1 text-sm text-mist leading-relaxed">{step.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((v, i) => (
              <Reveal key={v.title} delay={i * 0.1}>
                <div className="group relative overflow-hidden rounded-[20px] border border-gold/30 bg-white/5 p-8 backdrop-blur-md transition-all duration-300 hover:-translate-y-1.5 text-left">
      ...
      <section className="bg-white py-24 md:py-32">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-14 lg:grid-cols-2 items-center">
            <Reveal className="text-left space-y-3 font-display text-2xl sm:text-3xl md:text-[28px] font-bold leading-relaxed">
      ...
      {/* Join CTA */}
      <section className="relative overflow-hidden bg-navy py-20 border-t border-gold/20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="rounded-[24px] border border-gold/40 bg-white/5 p-10 sm:p-14 backdrop-blur-md text-center">
            ...
            <div className="mt-8">
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 rounded-full bg-gold px-8 py-4 text-sm font-bold text-navy transition hover:bg-white hover:scale-105 shadow-lg"
              >
